...
def evaluation_function(response, answer, params) -> EvaluationResponse:
    ...
    if eval_response_nlp.is_correct and eval_response_slm.is_correct:
        ...
        eval_response.is_correct = False
        eval_response.add_feedback(("feedback", "The response is ALMOST correct. But the student missed some key points. " + eval_response_nlp["feedback"]))
    elif eval_response_slm.is_correct and eval_response_nlp["metadata"]["similarity_value"] <= 0.75:
        eval_response.is_correct = False
        eval_response.add_feedback(("feedback", "The response is incorrect as the student missed some key points. " + eval_response_nlp["feedback"]))
    elif eval_response_nlp.is_correct:
        eval_response.is_correct = False
        eval_response.add_feedback(("feedback", "The response has pointed out all the key ideas, but its context is wrong."))
    else:
        eval_response.is_correct = False
        eval_response.add_feedback(("feedback", "The response is incorrect as its context is wrong. " + eval_response_nlp["feedback"]))
